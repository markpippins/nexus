-- =============================================================================
--  epistemologist-grant-v0.1.sql — Wave-2 grant event (DBA)
--
...
--  template header.
--
--  Ratified design for epistemologist:
--      {} domains; creates AND resolves questions, max 5 — co-owns
--      ambiguity_resolution with analyst. THE ONLY ACTUAL AUTHORITY IN
--      WAVE 2: resolution is boundary-constrained to the knowledge domain
--      (architect's reaffirmation); out_of_domain_resolution escalates to
--      {architect}. The boundary is what makes it ratifiable.
-- =============================================================================
...
DECLARE
    ...
    v_role                  text    := 'epistemologist';
    v_owns_domains          text[]  := ARRAY[]::text[];
    ...
    v_max_open_questions    integer := 5;
...
BEGIN
    SELECT count(*) INTO v_open   FROM nebula.roles_history
     WHERE name = 'epistemologist' AND valid_until = '9999-12-31 00:00:00+00'::timestamptz;
    ...
    JOIN nebula.roles_history c ON c.name = o.name
      ...
      AND c.valid_until = (SELECT max(valid_until) FROM nebula.roles_history
                           WHERE name = 'epistemologist'
                             AND valid_until <> '9999-12-31 00:00:00+00'::timestamptz)
    WHERE o.name = 'epistemologist'
    ...
    IF v_cq IS DISTINCT FROM TRUE
       OR v_rq IS DISTINCT FROM TRUE
       OR v_moq IS DISTINCT FROM 5
       OR v_domains IS DISTINCT FROM ARRAY[]::text[] THEN
        RAISE EXCEPTION 'GRANT verify: granted values do not match the ratified Wave-2 design'
            USING ERRCODE = 'P0001';
    END IF;

    RAISE NOTICE 'GRANT verified — epistemologist chain: closed=% open=%, handoff exact, ratified Wave-2 values pinned', v_closed, v_open;
END $$;
