-- =============================================================================
--  sound-technician-grant-v0.1.sql — Wave-2 grant event (DBA)
--
...
--  template header.
--
--  Ratified design for sound-technician:
--      {audio_feedback} domain only — completion-signal/TTS (R6).
--      No flags: operational domain ownership, no authority.
-- =============================================================================
...
DECLARE
    ...
    v_role                  text    := 'sound-technician';
    v_owns_domains          text[]  := ARRAY['audio_feedback'];
...
BEGIN
    SELECT count(*) INTO v_open   FROM nebula.roles_history
     WHERE name = 'sound-technician' AND valid_until = '9999-12-31 00:00:00+00'::timestamptz;
    ...
    JOIN nebula.roles_history c ON c.name = o.name
      ...
      AND c.valid_until = (SELECT max(valid_until) FROM nebula.roles_history
                           WHERE name = 'sound-technician'
                             AND valid_until <> '9999-12-31 00:00:00+00'::timestamptz)
    WHERE o.name = 'sound-technician'
    ...
    IF v_cq IS DISTINCT FROM false
       ...
       OR v_domains IS DISTINCT FROM ARRAY['audio_feedback'] THEN
        RAISE EXCEPTION 'GRANT verify: granted values do not match the ratified Wave-2 design'
            USING ERRCODE = 'P0001';
    END IF;

    RAISE NOTICE 'GRANT verified — sound-technician chain: closed=% open=%, handoff exact, ratified Wave-2 values pinned', v_closed, v_open;
END $$;
