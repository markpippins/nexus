-- =============================================================================
--  operator-grant-v0.1.sql — Wave-2 grant event (DBA)
--
...
--  template header.
--
--  Ratified design for operator:
--      {fleet_operations} domain only — UI fleet operations. Agenda
--      rights STAY with planner (architect's ruling); the operator
--      standing-lease holder is a service principal, not this role's
--      authority. No flags.
-- =============================================================================
...
DECLARE
    ...
    v_owns_domains          text[]  := ARRAY['fleet_operations'];
...
BEGIN
    SELECT count(*) INTO v_open   FROM nebula.roles_history
     WHERE name = 'operator' AND valid_until = '9999-12-31 00:00:00+00'::timestamptz;
    ...
    IF v_cq IS DISTINCT FROM false
       ...
       OR v_domains IS DISTINCT FROM ARRAY['fleet_operations'] THEN
        RAISE EXCEPTION 'GRANT verify: granted values do not match the ratified Wave-2 design'
            USING ERRCODE = 'P0001';
    END IF;

    RAISE NOTICE 'GRANT verified — operator chain: closed=% open=%, handoff exact, ratified Wave-2 values pinned', v_closed, v_open;
END $$;
