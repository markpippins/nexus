-- =============================================================================
--  critic-grant-v0.1.sql — Wave-2 grant event (DBA)
--
...
--  template header.
--
--  Ratified design for critic:
--      {} domains — advisory leverage comes from questioning, not
--      authority; questions-only (create, no resolve), max_open_questions=5.
-- =============================================================================
...
BEGIN
    SELECT count(*) INTO v_open   FROM nebula.roles_history
     WHERE name = 'critic' AND valid_until = '9999-12-31 00:00:00+00'::timestamptz;
    ...
    RAISE NOTICE 'GRANT verified — critic chain: closed=% open=%, handoff exact, ratified Wave-2 values pinned', v_closed, v_open;
END $$;
