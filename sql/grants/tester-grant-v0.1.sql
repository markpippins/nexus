-- =============================================================================
--  tester-grant-v0.1.sql — canonical close-then-insert grant event (DBA)
--
--  Second use of the corrected template (sql/grants/auditor-grant-v0.1.sql,
--  merged in #299) — first use with a TRUE capability boolean
--  (can_verify_work_requests). DRAFT — NOT APPLIED TO LIVE; apply waits on
--  explicit operator go. The two LESSONS below are template doctrine,
--  learned on the auditor grant (R2 79feb142), and apply to every grant
--  under the architect-ruled convention (thread 225dfbbb):
--
...
--
--  Provenance for this specific grant: DBA proposal (this PR), following
--  the auditor precedent (architect ratification, thread 225dfbbb): the
--  two standing merge conditions — the code has tests, and the tests are
--  passing — are a verification claim, and no role currently holds the
--  verification capability. tester v0.1 makes the tester role the
--  test-attestation authority: it may VERIFY work requests (boolean true)
--  and attest that evidence exists, but cannot greenlight, create or
--  resolve questions, or create agendas. Escalation triggers encode what
--  the tester must refuse to attest unilaterally and route to the
--  architect: greenlight pressure, WR-verification without evidence,
--  untested-merge pressure, and flaky/skipped tests.
--  (roles_history carries no granted_by column — provenance lives here,
--  in the NEBULA_AUDIT trail, and in the linked agent records.)
--
--  Repeatability note: this file is a GRANT-EVENT executor, not a one-shot
--  migration. Re-running closes the role's current open snapshot and opens
--  a successor with the values as edited — each run is one attributable
--  grant event, and the bitemporal chain accumulates (V175 makes that
--  possible). A run against a role that does not exist, or against the
--  pre-V175 shape, fails loudly (close UPDATE matches nothing / successor
--  INSERT hits the full unique) — never a silent no-op. The verify gate
--  makes every outcome explicit.
-- =============================================================================
...
DECLARE
    ...
    v_role                  text    := 'tester';
    v_owns_domains          text[]  := ARRAY['test-verification'];
    ...
    v_can_verify_wrs        boolean := TRUE;   -- the point of the grant
    v_max_open_questions    integer := 0;
    v_requires_approval     text[]  := ARRAY['architect'];
    v_escalates_to          text[]  := ARRAY['architect'];
    v_escalation_triggers   text[]  := ARRAY['greenlight_pressure',
                                             'wr_verification_without_evidence',
                                             'untested_merge_pressure',
                                             'flaky_or_skipped_tests'];
...
BEGIN
    SELECT count(*) INTO v_open FROM nebula.roles_history
     WHERE name = 'tester' AND valid_until = '9999-12-31 00:00:00+00'::timestamptz;
    ...
    WHERE o.name = 'tester' AND o.valid_until = '9999-12-31 00:00:00+00'::timestamptz
    ...
    RAISE NOTICE '✅ GRANT verified — tester chain: closed=% open=%, handoff exact, domains={%}',
