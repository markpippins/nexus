-- =============================================================================
--  sysadmin-grant-v0.1.sql — Wave-2 grant event (DBA)
--
...
--  template header.
--
--  Ratified design for sysadmin:
--      {host_operations} domain only — host, storage, USB-mount lifecycle.
...
DECLARE
    ...
    v_role                  text    := 'sysadmin';
    v_owns_domains          text[]  := ARRAY['host_operations'];
...
BEGIN
    SELECT count(*) INTO v_open   FROM nebula.roles_history
     WHERE name = 'sysadmin' AND valid_until = '9999-12-31 00:00:00+00'::timestamptz;
    ...
    IF v_cq IS DISTINCT FROM false
       ...
       OR v_domains IS DISTINCT FROM ARRAY['host_operations'] THEN
        RAISE EXCEPTION 'GRANT verify: granted values do not match the ratified Wave-2 design'
            USING ERRCODE = 'P0001';
    END IF;

    RAISE NOTICE 'GRANT verified — sysadmin chain: closed=% open=%, handoff exact, ratified Wave-2 values pinned', v_closed, v_open;
END $$;
