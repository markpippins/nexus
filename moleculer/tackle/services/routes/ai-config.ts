...
aiConfigRouter.post("/test", async (req, res) => {
  try {
    ...
    // Static format string — mirrored from the incumbent (CodeQL
    // js/tainted-format-string remediation, alert #586).
  ...
  try {
    ...
    const onDone = async (exitCode: number | null) => {
      ...
      try {
        ...
        console.error("[verify] %s post-exit update failed: %s", sessionId, e.message);
      }
      console.log("[verify] model=%s session=%s exit=%s → %s", model_id, sessionId, exitCode, success ? "VERIFIED" : "FAILED");
    };
    proc.on("exit", (code) => onDone(code));
    proc.on("error", (err) => {
      console.error("[verify] %s spawn error: %s", sessionId, err.message);
      onDone(null);
    });
    ...
    const timestamp = new Date().toISOString();
    // Static format string — mirrored from the incumbent (CodeQL
    // js/tainted-format-string remediation, alert #587).
