"""Hermetic tests for bin/sdk_drift_stamp_wrap.py.
...
passthrough — the same contract as test_users_bcrypt_drift_wrap.py, so
both timer series file records consistently (PR #527 parity, applied to
the SDK wrapper per the user request).

No database, no network: the checker and post-agent-record subprocess
calls are faked at the subprocess.run boundary, and the state file is
...
spec = importlib.util.spec_from_file_location(
    "sdk_drift_stamp_wrap", REPO / "bin" / "sdk_drift_stamp_wrap.py")
wrap = importlib.util.module_from_spec(spec)
sys.modules["sdk_drift_stamp_wrap"] = wrap
...
def check_ok():
    return _Result(0, "[STAMP] peb-kernel/spring: OK\n", "")


def check_drift():
    return _Result(1, "[DRIFT] peb-kernel/spring: stamp mismatch\n", "")


def check_tool_error():
    return _Result(2, "", "TOOL ERROR: tsp toolchain missing")
...
def test_first_clean_run_files_commissioning_record(monkeypatch, state_path):
    ...
    assert "series:sdk-drift-stamp" in fake.tags(recs[0])
...
def test_drift_files_inspection_record_then_same_day_suppressed(monkeypatch, state_path):
    ...
    assert "series:sdk-drift-stamp" in fake.tags(rec)
...
def test_series_contract_matches_bcrypt_wrapper_shape():
    """Lockstep pin: both wrappers' decision functions agree on the full
    state matrix (behavioral equality, not text equality)."""
    bcrypt_spec = importlib.util.spec_from_file_location(
        "users_bcrypt_drift_wrap_ref",
        REPO / "bin" / "users_bcrypt_drift_wrap.py")
    bcrypt = importlib.util.module_from_spec(bcrypt_spec)
    bcrypt_spec.loader.exec_module(bcrypt)
    for state, week in [
        ({}, "2026-W39"),
        ({"last_drift_recorded": "2026-09-20"}, "2026-W39"),
        ({"ever_green": True, "last_green_recorded": "2026-W39"}, "2026-W39"),
        ({"ever_green": True, "last_green_recorded": "2026-W38"}, "2026-W39"),
        ({"ever_green": False, "last_drift_recorded": "2026-09-24", "last_green_recorded": "2026-W37"}, "2026-W39"),
    ]:
        assert (wrap.decide_green_action(dict(state), week)
                == bcrypt.decide_green_action(dict(state), week)), (state, week)


def test_role_is_engineer_and_series_tag_present():
    assert "series:sdk-drift-stamp" in wrap.GREEN_TAGS
    assert "series:sdk-drift-stamp" in wrap.DRIFT_TAGS
