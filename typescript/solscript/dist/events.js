...
export function buildReadSetManifest(input) {
    const manifest = {
...
export function manifestToDict(m) {
    return { ...m, idempotencyKey: m.idempotencyKey };
}
export function keychainEventId(e) {
    return `${e.sourceNamespace}:${e.sourceEventId}`;
}
export function keychainEventToDict(e) {
    return {
        ...e,
        event_id: e.eventId,
...
function makeKeychainEvent(partial) {
    const full = {
...
export function buildEvaluationEvent(input) {
    return makeKeychainEvent({
        sourceNamespace: input.sourceNamespace ?? "sol-api",
        sourceEventId: input.sourceEventId,
