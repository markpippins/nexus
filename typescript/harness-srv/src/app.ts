// harness-srv — the Express app and its route surface.
//
// Extracted verbatim from index.ts (PR #575, tester review ecfe5977 GAP 1)
// so the global rate limiter and the containment guards are testable
// without binding a port. index.ts keeps the process lifecycle (listen,
// watchdog, signal handlers). Middleware order and every handler are
...
// (alerts #389/#607/#390). Same posture as nebula-srv's limiter.ts:
// 300 req/min/IP keeps normal operator/agent polling well clear of the
// ceiling while capping resource-exhaustion floods. Deliberately generous
// relative to job cost — execute routes are gated by admission + watchdog.
app.use(
  rateLimit({
    windowMs: 60 * 1000,
    max: 300,
...
export const PORT = parseInt(process.env.HARNESS_PORT || "3420");
export const WORK_DIR = process.env.HARNESS_WORK_DIR || "/home/codex/dev";
...
export const PROMPT_DIR = resolve(WORK_DIR, ".harness", "prompts");
...
export const LOG_FILE = join(LOG_DIR, "harness-srv.log");
...
app.post("/run", async (req, res) => {
  ...
  try {
    ...
    // (CodeQL js/resource-exhaustion remediation; lower bound per tester
    // review 269a6ca5 — a near-zero timeout would spin the supervisor).
